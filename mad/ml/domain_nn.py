...
class builder:
    ...
    def nestedcv(
        ...
        if model_type in ensemble_methods:
            ...
            train_nn = np.concatenate(train_collect)
            train_ctr_target = np.concatenate(train_targets)
            std = np.std(train_ctr_target)
            train_size = train_nn.shape[0]
            test_nn = np.array(X_test_select)
            ...
            hidden_layer_sizes=(100,100,100,100,25)
            if train_nn.shape[1] > 100:
                hidden_layer_sizes=(200,200,200,100,25)
            elif train_nn.shape[1] <= 5:
                # for friedman
                hidden_layer_sizes=(20,20,20,20,5)

            model = MLPClassifier(  random_state=1,
                                    batch_size = batch_size,
                                    max_iter=max_iter,
                                    hidden_layer_sizes = hidden_layer_sizes)
            model.fit(train_nn, labels)

            dists_nn_cosine.train(X = train_nn, model = model ) # 1 is in-domain, 0 for OOD
            # dist score is the probability to be OOD
            tr_distance_nn = dists_nn_cosine.predict(train_nn) # in domain training distances
            te_distance_nn = dists_nn_cosine.predict(test_nn)
            # assert False f""
