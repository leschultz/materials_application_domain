...
def distance_link(
    ...
    if dist_type == 'mahalanobis':
    ...
    elif dist_type == 'attention_metric_rev':
        ...
        if X_train.shape[1] < 2:
            ...
            keys = X_train
            # obtain cosine similarity range from 0 - 2 (2 means most similar)
            similarity = cdist(queries, keys, metric='cosine')
            denominator = np.sum(similarity, axis=1)  # row sum
    ...
    elif dist_type == 'gpr_err_bar':

        kernel = DotProduct() + WhiteKernel()
        gp = GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=10)
        gp.fit(X_train, y_train)
        y_pred, sigma = gp.predict(X_test, return_std=True)
        er_bar_length = 2 * 1.96 * sigma

        dists[append_name+dist_type] = er_bar_length
